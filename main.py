...
def main():
    ...
    for pu_config in config.POWERUP_CONFIG:
        ...
        if tex_id and w > 0 and h > 0:
            ...
            if not uvs:
                print(f"Aviso: Falha ao calcular UVs para '{pu_type}'")
                continue
    ...
    if not config.CLOUD_CONFIG:
        print("  - Nenhuma nuvem configurada, usando fallback (quadrados).")
    else:
        for cloud_conf in config.CLOUD_CONFIG:
            path = cloud_conf['path']
            cols = cloud_conf.get('cols', 1)
            rows = cloud_conf.get('rows', 1)
            ...
            if tex_id and w > 0 and h > 0:
                ...
                if uvs:
                    ...
                    print(f"Aviso: Falha ao calcular UVs para nuvem '{path}'")
            else:
                print(f"Aviso: Falha ao carregar textura para nuvem '{path}'")
    ...
    if config.trunk_texture_id:
        textures_to_delete.append(config.trunk_texture_id)
    if config.root_texture_id:
        textures_to_delete.append(config.root_texture_id)
    if config.ground_texture_id:
        textures_to_delete.append(config.ground_texture_id)
...
def render():
    ...
    current_time = pause.get_game_time()
    if config.invulnerable:
        ...
        current_status_y += status_y_offset
    if config.chainsaw_active or config.chainsaw_deactivation_pending:
        ...
        draw_text(-0.95, current_status_y, f"CHAINSAW: {pipes_left} pipes")
        current_status_y += status_y_offset
    if config.heavy_jump_active:
        remaining_time = max(0, config.heavy_jump_end_time - current_time)
        draw_text(-0.95, current_status_y, f"HEAVY JUMP: {remaining_time:.1f}s")
        current_status_y += status_y_offset
    ...
    if config.game_over:
        ...
        draw_text(-0.4, 0.0, "GAME PAUSED! Press ESC to Continue")
