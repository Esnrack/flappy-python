...
def key_callback(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS:
        ...
        if key == glfw.KEY_SPACE:
            if config.game_paused: return # Ignora se pausado
        ...
        if key == glfw.KEY_R and config.game_over:
            ...
            config.shrink_active = False; config.shrink_end_time = 0.0
            config.ground_offset_x = 0.0
            config.clouds.clear(); config.last_cloud_spawn_time = 0.0; config.next_cloud_spawn_interval = 0.0
            config.game_paused = False; config.pause_start_time = 0.0; config.total_pause_duration = 0.0