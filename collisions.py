...
def check_collision():
    ...
    bird_collision_half_w = bird_collision_w / 2.0; bird_collision_half_h = bird_collision_h / 2.0
    bird_bottom_edge = config.BIRD_Y - bird_collision_half_h; bird_top_edge = config.BIRD_Y + bird_collision_half_h
    bird_left_edge = config.BIRD_X - bird_collision_half_w; bird_right_edge = config.BIRD_X + bird_collision_half_w
    ...
    if bird_bottom_edge <= -0.9:
        ...
        config.bird_velocity = 0; return
    if bird_top_edge >= 1.0:
        if not config.invulnerable: handle_collision()
        if not config.game_over: config.BIRD_Y = 1.0 - bird_collision_half_h
        config.bird_velocity = min(0, config.bird_velocity); return
    ...
    for powerup in config.powerups:
        if not powerup.collected:
            ...
            closest_x = max(bird_left_edge, min(px, bird_right_edge)); closest_y = max(bird_bottom_edge, min(py, bird_top_edge))
            dist_sq = (px - closest_x)**2 + (py - closest_y)**2
            if dist_sq < (pr * pr):
                print(f"Coletou power-up: {powerup.type}"); powerup.collected = True
    ...
    for pipe in config.pipes:
        ...
        if horizontal_overlap:
            ...
            if hit_bottom_pipe or hit_top_pipe: handle_collision(); return