...
def handle_collision():
    ...
    if config.lives <= 0:
        ...
        config.invulnerable_time = config.get_game_time() + invulnerability_duration
...
def check_collision():
    ...
    # --- Calcula Hitbox AABB do Pássaro (com Shrink) ---
    # Aplica escala do shrink SE ativo
    current_size_scale = config.SHRINK_SCALE_FACTOR if config.shrink_active else 1.0

    # Calcula dimensões da hitbox (base * escala_hitbox * escala_shrink)
    bird_collision_w = config.BIRD_DRAW_WIDTH * config.BIRD_COLLISION_SCALE_W * current_size_scale
    # Usa aspect ratio para altura base, depois aplica escalas
        ...
        if not powerup.collected:
            ...
            if dist_sq < (pr * pr):
                ...
                if powerup.type == 'life':
                    ...
                    boost_duration = 5.0
                    config.invulnerable_time = config.get_game_time() + boost_duration
                    ...
                    config.heavy_jump_end_time = config.get_game_time() + config.HEAVYJUMP_DURATION_SECONDS
                    print(f"Heavy Jump ativado por {config.HEAVYJUMP_DURATION_SECONDS:.1f} segundos!")
                # --- ADICIONA LÓGICA PARA SHRINK ---
                elif powerup.type == 'shrink':
                    config.shrink_active = True
                    config.shrink_end_time = config.get_game_time() + config.SHRINK_DURATION_SECONDS
