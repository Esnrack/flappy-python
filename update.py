...
def update(delta_time):
    ...
    # --- Lógica Principal do Jogo (Só executa se o jogo estiver rodando e não pausado) ---
    if not config.game_started or config.game_over or config.game_paused:
        # Se o jogo terminou, verifica se bateu o recorde
        if config.game_over:
            update_high_score(config.score)
        return # Sai da função se o jogo não está ativo ou está pausado
