...
def draw_bird():
    ...
    if not config.bird_texture_id or not config.bird_frames_uv:
        ...
        glBegin(GL_QUADS)
        # Usa tamanho de colisão base para fallback
    ...
    current_draw_scale = config.SHRINK_SCALE_FACTOR if config.shrink_active else 1.0
    draw_w = config.BIRD_DRAW_WIDTH * current_draw_scale
    # Ajusta altura de desenho pelo aspect ratio do frame E pelo shrink
    # Preserva o aspect ratio original do frame, mesmo encolhido
    draw_h = (draw_w / config.bird_frame_aspect) if config.bird_frame_aspect > 0 else draw_w

    half_w = draw_w / 2.0
    half_h = draw_h / 2.0
    # --- Fim Cálculo Tamanho Desenho ---
    ...
    # Desenha o Quad mapeando os cantos da textura (V invertido)
    glBegin(GL_QUADS)
    glTexCoord2f(u0, v1); glVertex2f(x0, y0) # Inf Esq Vértice -> Sup Esq Textura V
    glTexCoord2f(u1, v1); glVertex2f(x1, y0) # Inf Dir Vértice -> Sup Dir Textura V
    glTexCoord2f(u1, v0); glVertex2f(x1, y1) # Sup Dir Vértice -> Inf Dir Textura V
    glTexCoord2f(u0, v0); glVertex2f(x0, y1) # Sup Esq Vértice -> Inf Esq Textura V
    glEnd()