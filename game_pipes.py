...
def update_pipes(delta_time):
    ...
    for pipe in config.pipes: # Move canos
        pipe['x'] -= config.PIPE_SPEED * config.speed_multiplier * delta_time
        if pipe.get('v_speed', 0) != 0:
            delta_y = pipe['v_speed'] * delta_time; new_bottom_h = pipe['bottom_height'] + delta_y
            max_bottom_limit_struct = max_safe_top_limit - config.PIPE_GAP
            if new_bottom_h < min_safe_bottom_limit: new_bottom_h = min_safe_bottom_limit; pipe['v_speed'] *= -1
            elif new_bottom_h > max_bottom_limit_struct: new_bottom_h = max_bottom_limit_struct; pipe['v_speed'] *= -1
            pipe['bottom_height'] = new_bottom_h; pipe['top_height'] = new_bottom_h + config.PIPE_GAP
    config.pipes[:] = [pipe for pipe in config.pipes if pipe['x'] + config.PIPE_WIDTH > -1.0] # Remove canos
    current_time = time.time(); time_since_last_pipe = current_time - config.last_pipe_time # Gera novos
    ...
    if (config.game_started and not config.game_over and time_since_last_pipe > spawn_interval_adjusted):
        overall_min_bottom_h = -1.0 + 0.2; overall_max_bottom_h = 1.0 - 0.2 - config.PIPE_GAP
        reference_height = 0.0; previous_was_moving = False
        if config.pipes:
            previous_pipe_ref = config.pipes[-1]; previous_v_speed_ref = previous_pipe_ref.get('v_speed', 0)
            previous_was_moving = (previous_v_speed_ref != 0); time_to_estimate = spawn_interval_adjusted
        ...
        else: reference_height = (overall_max_bottom_h + overall_min_bottom_h) / 2 - 0.1
        ...
        target_min = reference_height - height_change_limit; target_max = reference_height + height_change_limit
        current_min_h = max(overall_min_bottom_h, target_min); current_max_h = min(overall_max_bottom_h, target_max)
        ...
        current_gap = config.PIPE_GAP
        if config.chainsaw_active: current_gap += config.CHAINSAW_GAP_INCREASE
        top_pipe_h_structural = bottom_pipe_h + config.PIPE_GAP
        pipe_v_speed = 0.0; allow_move = True
        ...
        pipe_generated_this_frame = True; config.last_pipe_time = current_time
    if pipe_generated_this_frame and len(config.pipes) >= 2: # Gera Powerup
        ...
        if random.random() < 0.10:
            if config.POWERUP_TYPES:
                ...
                powerup_y_min = path_midpoint_y - half_range; powerup_y_max = path_midpoint_y + half_range
                powerup_y = random.uniform(powerup_y_min, powerup_y_max)
                min_y_bound = -1.0 + config.POWERUP_DRAW_SIZE * 0.5; max_y_bound = 1.0 - config.POWERUP_DRAW_SIZE * 0.5
    ...
    for pipe in config.pipes: # Pontuação
         ...
         if not pipe.get('scored', False) and config.BIRD_X > pipe_right_edge:
              config.score += 1; pipe['scored'] = True
              if config.chainsaw_active and not config.chainsaw_deactivation_pending:
                  ...
                  if config.chainsaw_pipes_remaining <= 0:
                      config.chainsaw_deactivation_pending = True; config.chainsaw_last_pipe_ref = pipe